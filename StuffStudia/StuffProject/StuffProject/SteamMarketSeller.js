...
(function ($, async, g_rgAppContextData, g_strInventoryLoadURL, g_rgWalletInfo) {
    ...
    //	"descriptions":[{"value":""}],
    //	"owner_actions":[{"name":"View badge progress","link":"https://steamcommunity.com/my/gamecards/245070/"}],
    ...
    SteamMarket.prototype.getListings = function (item, callback/*err, listings*/) {
        try {
            var url = 'https://steamcommunity.com/market/listings/';
    ...
    function calculateSellPrice(history, listings, item) {
        return calculateSellPrice_safe(history, listings, item);
    ...
    function LWMA(prices) {
        var N = prices.length;
        var numerator = 0.0;
        for (var i = 0; i < N; i++)
            numerator += (i + 1) * prices[i][1];
        var denumerator = N * (N + 1.0) / 2;
        var lwma = numerator / denumerator;
        return lwma;
    }

    function calculateSellPrice_safe(history, listings, item) {
        // Fairly safe sell
        ...
        var ii = history.length - 1;
        var sum = 0.0;
        var tmpDate = new Date(history[ii][0]).getTime();
        //log(item.name);
        while (ii > 0 && tmpDate > oneWeekAgo) {
            ii--;
            tmpDate = new Date(history[ii][0]).getTime();
        }

        var arr = history.slice(ii, history.length);
        var max = 0.0;
        //log("Total length: " + history.length + " current length: " + arr.length);
        if (typeof arr !== 'undefined' && false) {
            if (arr.length > 0) {
                for (var i = 0; i < arr.length; i++) {
                    var curPrice = arr[i][1];
                    sum += curPrice;
                    if (max < curPrice) max = curPrice;
                }
                var mean = sum / arr.length;
                var lwmean = Math.trunc(LWMA(arr));
                log(item.name + " -> " + lwmean);
                log("man: " + max);
                //return lwmean;

                var price = (mean + max) / 2.0;
                log("price " + price);
                //price = lwmean;

                var fl = listings[Object.keys(listings)[0]];

                if (price < (fl.converted_price + fl.converted_fee - 1)) {
                    price = fl.converted_price - 1;
                }
        ...
        var itemQueue = async.queue(function (item, next) {
            ...
            market.getPriceHistory(item, function (err, history) {
                ...
                market.getListings(item, function (err, listings) {
                    ...
                    var sellPrice = calculateSellPrice(history, listings, item);
