...
// @require        https://raw.githubusercontent.com/caolan/async/v1.5.2/lib/async.js
// @grant          none
// ==/UserScript==
        ...
        var itemQueue = async.queue(function(item, next) {
            ...
            market.getPriceHistory(item, function(err, history) {
                ...
                market.getListings(item, function(err, listings) {
                    ...
                    if ((sellPrice==0)&&(history.length>0)) {
                        history.sort(function(a, b){return b-a});
                        console.log('Array[0] = '+market.getPriceBeforeFees(history[0]));
                        /* sellQueue.push({
                            item: item,
                            sellPrice: market.getPriceBeforeFees(history[0])
                        });*/
                    }
                    next();
                });
